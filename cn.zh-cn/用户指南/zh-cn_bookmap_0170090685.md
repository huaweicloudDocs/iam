# 用户指南

-   [使用前必读](使用前必读.md)
-   [IAM用户IAM用户](IAM用户.md)
    -   [创建IAM用户创建IAM用户](创建IAM用户.md)
    -   [给IAM用户授权给IAM用户授权](给IAM用户授权.md)
    -   [IAM用户登录IAM用户登录](IAM用户登录.md)
    -   [修改IAM用户信息修改IAM用户信息](修改IAM用户信息.md)
    -   [修改IAM用户密码修改IAM用户密码](修改IAM用户密码.md)
    -   [管理IAM用户访问密钥管理IAM用户访问密钥](管理IAM用户访问密钥.md)

-   [用户组及授权](用户组及授权.md)
    -   [创建用户组并授权](创建用户组并授权.md)
    -   [用户组添加用户](用户组添加用户.md)
    -   [查看或修改用户组](查看或修改用户组.md)
    -   [依赖角色的授权方法](依赖角色的授权方法.md)

-   [权限](权限.md)
    -   [权限基本概念](权限基本概念.md)
    -   [申请基于策略的访问控制公测](申请基于策略的访问控制公测.md)
    -   [角色](角色.md)
    -   [策略](策略.md)
    -   [系统策略更名详情](系统策略更名详情.md)
    -   [自定义策略](自定义策略.md)
        -   [创建自定义策略](创建自定义策略.md)
        -   [修改、删除自定义策略](修改-删除自定义策略.md)
        -   [自定义策略使用样例](自定义策略使用样例.md)


-   [项目](项目.md)
-   [委托](委托.md)
    -   [委托其他华为云账号管理资源](委托其他华为云账号管理资源.md)
        -   [基本流程](基本流程.md)
        -   [创建委托（委托方操作）](创建委托（委托方操作）.md)
        -   [（可选）分配委托权限（被委托方操作）](（可选）分配委托权限（被委托方操作）.md)
        -   [切换角色（被委托方操作）](切换角色（被委托方操作）.md)

    -   [委托其他云服务管理资源](委托其他云服务管理资源.md)

-   [账号安全设置](账号安全设置.md)
    -   [账号安全设置概述](账号安全设置概述.md)
    -   [账号设置](账号设置.md)
    -   [敏感操作](敏感操作.md)
    -   [登录验证策略](登录验证策略.md)
    -   [密码策略](密码策略.md)
    -   [访问控制](访问控制.md)

-   [身份提供商](身份提供商.md)
    -   [身份提供商概述](身份提供商概述.md)
    -   [联邦身份认证的基本流程和配置步骤](联邦身份认证的基本流程和配置步骤.md)
    -   [步骤1：创建身份提供商](步骤1-创建身份提供商.md)
    -   [步骤2：配置身份转换规则](步骤2-配置身份转换规则.md)
    -   [步骤3：配置企业管理系统登录入口](步骤3-配置企业管理系统登录入口.md)
    -   [身份转换规则详细说明](身份转换规则详细说明.md)

-   [自定义身份代理](自定义身份代理.md)
-   [多因素认证与虚拟MFA](多因素认证与虚拟MFA.md)
    -   [多因素认证](多因素认证.md)
    -   [虚拟MFA](虚拟MFA.md)

-   [使用伙伴云](使用伙伴云.md)
-   [查看IAM操作记录](查看IAM操作记录.md)
    -   [开通云审计服务](开通云审计服务.md)
    -   [查看IAM的云审计日志](查看IAM的云审计日志.md)

-   [修订记录](修订记录.md)

