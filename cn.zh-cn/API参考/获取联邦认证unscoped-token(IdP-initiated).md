...
1.  在浏览器的地址栏输入并跳转链接：[https://idp.example.org/idp/profile/SAML2/Unsolicited/SSO?providerId=iam.example.com](https://idp.example.org/idp/profile/SAML2/Unsolicited/SSO?providerId=iam.example.com)。
...
3.  该链接可打开身份提供商登录页面，根据需要输入映射规则中的用户名（支持免密登录），单击登录，跳入认证页面后按F12,单击认证页面的accept。从下图所示的POST中获取SAMLResponse。
    ...
    ![](figures/zh-cn_image_0226093583.png)
