...
在[如何调用API](构造请求.md)中以调用[获取用户Token](zh-cn_topic_0163397778.md)接口为例，说明了如何调用API。获取Token后，再调用其他接口时，您需要在请求消息头中添加“X-Auth-Token”，其值即为获取到的Token。例如Token值为“ABCDEFJ....“，则调用接口时将“X-Auth-Token: ABCDEFJ....“加到请求消息头即可，如下所示。
